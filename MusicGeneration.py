...
rel_path = "midi\sonate_01_(c)hisamori.mid"
abs_file_path = src_file_path + '\\' + rel_path

# Enter here the location of your MIDI file:
file = converter.parse("C:\Cassiopée\Cassiopee-33-Repository\midi\sonate_01_(c)hisamori.mid")
count = 0
everything = ""
notes = []
...
for element in file.recurse().notes:
    if(element.isNote):
        ...
        note += str(element.duration.quarterLength)
        everything += note
        everything += ' '
        count += 1
        notes.append(note)
...
if(is_generating):
    ...
    weight_filename = "weights-improvement-5.7957-20.hdf5"
    weight_filepath = src_file_path + "/weights/" + weight_filename
    model.load_weights(weight_filepath)

if(is_training and trainingHasWeights):
    # Load the network weights
    weight_filename = "weights-improvement-1.4666-19.hdf5"
    weight_filepath = src_file_path + "/weights/" + weight_filename
    model.load_weights(weight_filepath)
...
if(is_training):
    ...
    filepath=src_file_path + "/weightsMusic/" + "weightsMusic-improvement-{loss:.4f}-{epoch:02d}.hdf5"
